...
static const TestData test_data[] = {
        ...
        },
        {
                "4ccd089b28ff96da9db6c346ec114e0f5b8a319f35aba624da8cf6ed4fb8a6fb",
                "3d4017c3e843895a92b70aa74d1b7ebc9c982ccf2ec4968cc0cd55f12af4660c",
                "",
        ...
        },
        {
                "c5aa8df43f9f837bedb7442f31dcb7b166d38535076f094b85ce3a2e0b4458f7",
                "fc51cd8e6218a1a38da47ed00230f0580816ed13ba3303ac5deb911548908025",
                "",
...
int main(void)
{
    ...
    expected_proof  = (unsigned char *) sodium_malloc(crypto_vrf_ietfdraft12_PROOFBYTES_BATCHCOMPAT);
    ...
    unsigned char proof[crypto_vrf_ietfdraft12_PROOFBYTES_BATCHCOMPAT];
    ...
    assert(crypto_vrf_ietfdraft12_PROOFBYTES_BATCHCOMPAT == 128);
    ...
    for (i = 0U; i < (sizeof test_data) / (sizeof test_data[0]); i++) {
        ...
        sodium_hex2bin(expected_proof, crypto_vrf_ietfdraft12_PROOFBYTES_BATCHCOMPAT,
        ...
        if (crypto_vrf_ietfdraft12_prove_batchcompat(proof, sk, messages[i], i) != 0){
            printf("crypto_vrf_prove() error: [%u]\n", i);
        }
        if (memcmp(expected_proof, proof, crypto_vrf_ietfdraft12_PROOFBYTES_BATCHCOMPAT) != 0){
            printf("proof error: [%u]\n", i);
            printhex("\tWanted: ", expected_proof, crypto_vrf_ietfdraft12_PROOFBYTES_BATCHCOMPAT);
            printhex("\tGot:    ", proof, crypto_vrf_ietfdraft12_PROOFBYTES_BATCHCOMPAT);
        }
        if (crypto_vrf_ietfdraft12_verify_batchcompat(output, expected_pk, proof, messages[i], i) != 0){
        ...
        if (i > 0) {
            if (crypto_vrf_ietfdraft12_verify_batchcompat(output, expected_pk, proof, messages[i], i-1) == 0){
                printf("verify succeeded with truncated message: [%u]\n", i);
            }
        }

        if (crypto_vrf_ietfdraft12_proof_to_hash_batchcompat(output, proof) != 0){
